		...
		<div class="form-group search-currency-dropdown">
			<select name="from_wallet" class="form-control form-control-sm">
				...
				@foreach($wallets as $key => $value)
					<option value="{{ $value->id }}" {{ isset($search['from_wallet']) && $search['from_wallet'] == $value->id ? 'selected' : '' }}>
						{{ __(optional($value->currency)->code) }}
						- {{ __(optional($value->currency)->name) }} </option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="col-md-2">
		<div class="form-group search-currency-dropdown">
			<select name="to_wallet" class="form-control form-control-sm">
				...
				@foreach($wallets as $key => $value)
					<option value="{{ $value->id }}" {{ isset($search['to_wallet']) && $search['to_wallet'] == $value->id ? 'selected' : '' }}>
						{{ __(optional($value->currency)->code) }}
						- {{ __(optional($value->currency)->name) }} </option>
				@endforeach
			</select>
	...
	<div class="col-md-2">
		<div class="form-group search-currency-dropdown">
			<select name="status" class="form-control form-control-sm">
				...
				<option value="1" {{ isset($search['status']) && $search['status'] == '1' ? 'selected' : '' }}>@lang('Completed')</option>
			</select>
		</div>
	</div>
	<div class="col-md-2">
